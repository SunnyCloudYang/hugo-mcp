...
@mcp.tool(name="check_hugo_installation", description="Check if Hugo is installed and get its version")
async def check_hugo_installation() -> Dict[str, Any]:
    try:
        ...
        return {"status": "error", "message": f"Hugo is not installed or not in PATH: {str(e)}"}
    except FileNotFoundError:
        return {"status": "error", "message": "Hugo is not installed or not in PATH"}

@mcp.tool(name="install_hugo", description="Install Hugo using the appropriate method for the current OS")
async def install_hugo(version: str = "latest") -> Dict[str, str]:
    ...
    try:
        if system == "darwin":  # macOS
            ...
            return {"status": "success", "message": "Hugo installed via Homebrew"}
        elif system == "linux":
            # Try to detect package manager
            if shutil.which("apt"):
                subprocess.run(["sudo", "apt", "update"], check=True)
                subprocess.run(["sudo", "apt", "install", "-y", "hugo"], check=True)
                return {"status": "success", "message": "Hugo installed via apt"}
            elif shutil.which("dnf"):
                subprocess.run(["sudo", "dnf", "install", "-y", "hugo"], check=True)
                return {"status": "success", "message": "Hugo installed via dnf"}
            elif shutil.which("yum"):
                subprocess.run(["sudo", "yum", "install", "-y", "hugo"], check=True)
                return {"status": "success", "message": "Hugo installed via yum"}
            else:
                return {"status": "error", "message": "Unsupported Linux distribution. Please install Hugo manually."}
        elif system == "windows":
            return {"status": "error", "message": "Windows installation not supported via this tool. Please install Hugo manually from https://gohugo.io/installation/windows/"}
        else:
            return {"status": "error", "message": f"Unsupported operating system: {system}"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Installation failed: {str(e)}"}

@mcp.tool(name="check_go_installation", description="Check if Go is installed and get its version")
async def check_go_installation() -> Dict[str, Any]:
    try:
        result = subprocess.run(["go", "version"], capture_output=True, text=True, check=True)
        return {"status": "success", "version": result.stdout.strip()}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Go is not installed or not in PATH: {str(e)}"}
    except FileNotFoundError:
        return {"status": "error", "message": "Go is not installed or not in PATH"}

@mcp.tool(name="install_go", description="Install Go using the appropriate method for the current OS")
    ...
    try:
        if system == "darwin":  # macOS
            ...
            if shutil.which("apt"):
                ...
                return {"status": "error", "message": "Unsupported Linux distribution. Please install Go manually."}
        elif system == "windows":
            return {"status": "error", "message": "Windows installation not supported via this tool. Please install Go manually from https://golang.org/dl/"}
        else:
            return {"status": "error", "message": f"Unsupported operating system: {system}"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Installation failed: {str(e)}"}
...
@mcp.tool(name="check_git_installation", description="Check if Git is installed and get its configuration")
async def check_git_installation() -> Dict[str, Any]:
    try:
        ...
        git_user_name = subprocess.run(["git", "config", "user.name"], capture_output=True, text=True)
        git_user_email = subprocess.run(["git", "config", "user.email"], capture_output=True, text=True)
        
        # Get git default branch
        git_default_branch = subprocess.run(["git", "config", "--global", "init.defaultBranch"], capture_output=True, text=True)
        
        return {
            "status": "success",
            "version": git_version.stdout.strip(),
            "user": {
                "name": git_user_name.stdout.strip() if git_user_name.returncode == 0 else None,
                "email": git_user_email.stdout.strip() if git_user_email.returncode == 0 else None
            },
            "default_branch": git_default_branch.stdout.strip() if git_default_branch.returncode == 0 else "main"
        }
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Git is not installed or not in PATH: {str(e)}"}
    except FileNotFoundError:
        return {"status": "error", "message": "Git is not installed or not in PATH"}
    except Exception as e:
        return {"status": "error", "message": f"Unexpected error: {str(e)}"}

@mcp.tool(name="install_git", description="Install Git using the appropriate method for the current OS")
    ...
    try:
        if system == "darwin":  # macOS
            ...
            if shutil.which("apt"):
                ...
                return {"status": "error", "message": "Unsupported Linux distribution. Please install Git manually."}
        elif system == "windows":
            return {"status": "error", "message": "Windows installation not supported via this tool. Please install Git manually from https://git-scm.com/download/win"}
        else:
            return {"status": "error", "message": f"Unsupported operating system: {system}"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Installation failed: {str(e)}"}
...
async def configure_git(name: str, email: str) -> Dict[str, str]:
    try:
        ...
        return {"status": "success", "message": f"Git configured with name '{name}' and email '{email}'"}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Failed to configure Git: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"Unexpected error: {str(e)}"}
...
async def create_site(
    ...
    try:
        ...
        # Create the site
        subprocess.run(["hugo", "new", "site", site_name], check=True)
        ...
        if theme:
            # Check if we should use git submodule or Hugo modules
            use_modules = False  # Default to git submodule
            ...
            subprocess.run(["git", "submodule", "add", f"https://github.com/{theme}", f"themes/{theme.split('/')[-1]}"], check=True)
            ...
            # Check for exampleSite in the theme
            if use_example_site:
                example_site_path = Path(f"themes/{theme_name}/exampleSite")
                if example_site_path.exists():
                    # Get git user info for personalization
                    git_info = await check_git_installation()
                    author_name = git_info.get("user", {}).get("name", "Your Name")
                    author_email = git_info.get("user", {}).get("email", "your.email@example.com")
                    
                    # Copy exampleSite content to the site root
                    for item in example_site_path.glob("**/*"):
                        if item.is_file():
                            # Calculate relative path
                            rel_path = item.relative_to(example_site_path)
                            target_path = site_path / rel_path
                    ...
                    # Update config with git user info
                    for config_file in config_files:
                        if os.path.exists(config_file):
                            if config_file.endswith(".toml"):
                                with open(config_file, "r") as f:
                                ...
                                content = content.replace('author = ""', f'author = "{author_name}"')
                                content = content.replace('email = ""', f'email = "{author_email}"')
        ...
        return {"status": "error", "message": f"Failed to create Hugo site: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"Unexpected error: {str(e)}"}
...
async def create_post(
    ...
    try:
        ...
        if not os.path.isdir(site_path):
            return {"status": "error", "message": f"Site path '{site_path}' does not exist"}
...
async def start_preview(
    ...
    try:
        ...
        if process.poll() is not None:
            ...
            return {"status": "error", "message": f"Server failed to start: {error_output}"}
...
async def build_site(
    ...
    try:
        ...
        return {"status": "error", "message": f"Build failed: {e.stderr}"}
    except Exception as e:
        return {"status": "error", "message": f"Unexpected error: {str(e)}"}
...
async def deploy_site(
    ...
    try:
        ...
        if platform.lower() == "github-pages":
            return await _deploy_to_github_pages(site_path, destination, branch, commit_message, api_key)
        elif platform.lower() == "netlify":
            return await _deploy_to_netlify(site_path, destination, api_key, additional_options)
        elif platform.lower() == "vercel":
            return await _deploy_to_vercel(site_path, destination, api_key, additional_options)
        elif platform.lower() == "custom" and remote_url:
            return await _deploy_to_custom(site_path, destination, remote_url, branch, commit_message)
        else:
...
async def _deploy_to_github_pages(
    ...
    try:
        ...
        return {"status": "error", "message": f"GitHub Pages deployment failed: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"GitHub Pages deployment failed: {str(e)}"}
...
async def _deploy_to_netlify(
    ...
    try:
        ...
        if additional_options and "production" in additional_options and additional_options["production"]:
            cmd.append("--prod")
        ...
        return {"status": "error", "message": f"Netlify deployment failed: {e.stderr}"}
    except Exception as e:
        return {"status": "error", "message": f"Netlify deployment failed: {str(e)}"}
...
async def _deploy_to_vercel(
    ...
    try:
        ...
        return {"status": "error", "message": f"Vercel deployment failed: {e.stderr}"}
    except Exception as e:
        return {"status": "error", "message": f"Vercel deployment failed: {str(e)}"}
...
async def _deploy_to_custom(
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["git", "remote", "set-url", "deploy", remote_url], check=True)
        
        # Push to remote
        subprocess.run(["git", "push", "deploy", f"HEAD:{branch}", "--force"], check=True)
        ...
        return {"status": "error", "message": f"Custom deployment failed: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"Custom deployment failed: {str(e)}"}

@mcp.tool(name="list_themes", description="List available Hugo themes from the official Hugo themes website")
async def list_themes() -> Dict[str, Any]:
    try:
        ...
        if response.status_code != 200:
            return {"status": "error", "message": f"Failed to fetch themes: HTTP {response.status_code}"}
        ...
        for item in theme_items:
            ...
            theme_url = f"https://github.com/gohugoio/hugoThemes/tree/master/themes{theme_path}"
        ...
        return {"status": "error", "message": f"Failed to list themes: {str(e)}"}

@mcp.tool(name="install_theme", description="Install a Hugo theme using git submodule or Hugo modules")
    ...
    try:
        ...
        if use_modules:
            ...
            if not os.path.exists("go.mod"):
                ...
                subprocess.run(["hugo", "mod", "init", f"github.com/{site_name}"], check=True)
            ...
            # Add the theme as a git submodule
            subprocess.run(["git", "submodule", "add", theme_url, f"themes/{theme_name}"], check=True)
...
async def update_theme(site_path: str, theme_name: str, use_modules: bool = False) -> Dict[str, Any]:
    try:
        # Validate site path
        if not os.path.isdir(site_path):
            return {"status": "error", "message": f"Site path '{site_path}' does not exist"}
        ...
        if use_modules:
            ...
            subprocess.run(["git", "submodule", "update", "--remote", f"themes/{theme_name}"], check=True)
            return {"status": "success", "theme": theme_name, "method": "git_submodule"}
    except subprocess.CalledProcessError as e:
...
async def list_content(site_path: str, content_type: Optional[str] = None) -> Dict[str, Any]:
    try:
        # Validate site path
        if not os.path.isdir(site_path):
            return {"status": "error", "message": f"Site path '{site_path}' does not exist"}
        ...
        if not os.path.isdir(content_dir):
            return {"status": "error", "message": f"Content directory '{content_dir}' does not exist"}
...
@mcp.tool(name="get_theme_details", description="Get detailed information about a specific Hugo theme")
async def get_theme_details(theme_name: str) -> Dict[str, Any]:
    try:
        ...
        if not theme_info:
            return {"status": "error", "message": f"Theme '{theme_name}' not found on the Hugo themes website"}
        ...
        for link in demo_links:
            href = link.get('href', '')
            if 'github.com' not in href and not href.startswith('https://themes.gohugo.io'):
                demo_url = href
                break
        
        # Extract installation instructions
        installation = ""
        install_section = detail_soup.find(string=re.compile(r'Installation|Install', re.IGNORECASE))
        if install_section:
